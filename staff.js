...
async function renderStaffExecutionDetail() {
  ...
  // Fetch sorted status from Supabase for this reservation
  let staffExecDayProcSorted = {};
  try {
    const sb = window.supabaseClient;
    if (sb) {
      const { data } = await sb.from('reservation_exec_sorted')
        .select('*')
        .eq('reservation_id', res.id)
        .single();
      if (data) {
        staffExecDayProcSorted = {
          food:           data.sorted_food          || false,
          equipment:      data.sorted_equipment     || false,
          decoration:     data.sorted_decoration    || false,
          personnel:      data.sorted_personnel     || false,
          transportation: data.sorted_transportation || false
        };
      }
    }
  } catch (e) {
    // Silently ignore — sorted boxes will safely default to unsorted
  }
  ...
  // Title = package name
  const execPkgName = res.packageName || res.package || res.type || (res.client + '\u2019s Event');

  // Resource preparation sorted status boxes (read-only)
  ...
  const sortedBoxesHtml = `
    <div style="background:var(--bg3);border:1px solid var(--border);border-radius:12px;padding:16px 20px;margin-bottom:16px;">
      <div style="font-size:11px;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px;">Resource Preparation Status</div>
      <div style="display:flex;flex-direction:column;gap:8px;">
        ${STAFF_EXEC_PROC_CATS.map(c => {
          const isSorted = staffExecDayProcSorted[c.key] || false;
          return `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-radius:8px;border:1px solid ${isSorted ? 'rgba(39,174,96,0.4)' : 'rgba(192,57,43,0.4)'};">
            <span style="font-size:13px;font-weight:600;color:var(--cream);">${c.label}</span>
            <span style="font-size:11px;font-weight:700;color:${isSorted ? 'var(--green)' : 'var(--red)'};">${isSorted ? 'Sorted' : 'Not Sorted'}</span>
          </div>`;
        }).join('')}
      </div>
    </div>`;
  ...
  detailView.innerHTML = `
    <div style="padding:28px; border-bottom:1px solid var(--border); background:rgba(255,255,255,0.01);">
      <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:20px;">
        <div>
          <h2 style="font-family:'Arial';font-size:26px;color:var(--gold);margin-bottom:4px;">${execPkgName}</h2>
          <p style="font-size:13px;color:var(--text-dim);">${res.client} &mdash; ${res.date}</p>
        </div>
