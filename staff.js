...
function checkAuth() {
  ...
  try {
    ...
    document.getElementById('admin-name').textContent = u.name || 'Staff Member';
  } catch(e) {}
}
...
async function loadData() {
  ...
  try {
    ...
    // Render staff-accessible sections only
    renderInventory();
...
// ===== ADMIN CHAT SUPPORT LOGIC (Shared with Staff) =====
let activeCustomerId = null;
let activeChatUnsub = null;
let conversationsUnsub = null;

function initAdminChat() {
  if (conversationsUnsub) return; // already listening
  ...
  list.innerHTML = convos.map(c => {
    ...
    return `
      <div class="inbox-item ${activeCustomerId === c.uid ? 'active' : ''} ${c.hasNew ? 'unread' : ''}" onclick="openConversation('${c.uid}', '${c.name.replace(/'/g, "\\'")}')">
        <div class="ii-name">
          ${c.name} 
          ${c.hasNew ? '<span class="ii-dot"></span>' : ''}
        </div>
...
async function openConversation(uid, name) {
  ...
  renderChatInbox([]); // refresh list to show active
...
// ====================================================================
// STAFF SCHEDULING VIEW  (read-only, driven by admin's EIM_SCHEDULES)
// ====================================================================
function renderStaffSchedulingView() {
  ...
  const labelEl = document.getElementById('staff-today-label');
  const countEl = document.getElementById('staff-event-count');
  const container = document.getElementById('staff-sched-container');
  if (!container) return;

  if (labelEl) labelEl.textContent = dateLabel;

  // EIM_SCHEDULES lives in admin.js — if staff.js is loaded standalone it won't exist, so guard it
  const schedules = (typeof EIM_SCHEDULES !== 'undefined') ? EIM_SCHEDULES : [];
  const RESERVS   = (typeof RESERVATIONS  !== 'undefined') ? RESERVATIONS  : [];

  const todayScheds = schedules.filter(s => s.eventDate === today);
  const todayEvents = RESERVS.filter(r => {
    const d = r.date ? new Date(r.date).toISOString().split('T')[0] : '';
    return d === today && ['confirmed','preparing','on-going'].includes(r.status);
  });
  ...
  if (todayEvents.length === 0) {
    container.innerHTML = `
      <div style="text-align:center; padding:60px 20px; color:var(--text-dim);">
        ...
        <div style="font-size:16px; font-family:'Playfair Display',serif; font-weight:700; color:var(--cream); margin-bottom:8px;">No Events Today</div>
        <div style="font-size:13px; max-width:380px; margin:0 auto; line-height:1.6;">There are no confirmed events scheduled for today. Check back here on event days.</div>
      </div>`;
    return;
  }

  const getStatusClass = s => ({ 'Fully Assigned':'confirmed', 'Partially Assigned':'pending', 'Insufficient':'critical', 'Unassigned':'cancelled' }[s] || 'pending');

  let html = '';

  // ---- Per-event cards ----
  todayEvents.forEach(ev => {
    const sch = todayScheds.find(s => s.eventId === ev.id);

    // Staff group computation (mirrors admin logic)
    const pax = parseInt(ev.pax) || 50;
    let scale, counts;
    if      (pax <= 50)  { scale='Light';   counts={setup:3,tableware:3,catering:2,av:2}; }
    else if (pax <= 100) { scale='Medium';  counts={setup:5,tableware:5,catering:3,av:2}; }
    else if (pax <= 200) { scale='Heavy';   counts={setup:8,tableware:7,catering:5,av:3}; }
    else                 { scale='Massive'; counts={setup:12,tableware:10,catering:7,av:4}; }
    const mult = {'Wedding':1.2,'Corporate':1.0,'Birthday Party':0.9,'Family Gathering':0.85}[ev.type] || 1.0;
    const groups = [
      { label:'Group A — Setup',      icon:'&#x1F3D7;', color:'#c49a3c', n:Math.max(2,Math.round(counts.setup     * mult)) },
      { label:'Group B — Tableware',  icon:'&#x1F37D;', color:'#7c6fcd', n:Math.max(2,Math.round(counts.tableware * mult)) },
      { label:'Group C — Catering',   icon:'&#x1F373;', color:'#2d8a4e', n:Math.max(2,Math.round(counts.catering  * mult)) },
      { label:'Group D — AV &amp; Lighting', icon:'&#x1F4A1;', color:'#d97706', n:Math.max(2,Math.round(counts.av      * mult)) },
    ];
    const totalStaff = groups.reduce((a,g) => a + g.n, 0);

    html += `
      <div class="panel" style="margin-bottom:20px;">
        <div class="panel-hdr">
          <div>
            <div class="panel-title">${ev.client}</div>
            <div class="panel-sub">${ev.pax} pax &middot; ${ev.packageName||ev.type} &middot; Scale: <strong style="color:var(--gold);">${scale}</strong></div>
          </div>
          <div style="display:flex; align-items:center; gap:10px;">
            <div style="text-align:right;">
              <div style="font-size:10px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;">Total Staff</div>
              <div style="font-size:22px;font-family:'Playfair Display',serif;font-weight:700;color:var(--cream);">${totalStaff}</div>
            </div>
            ${sch ? `<span class="badge ${getStatusClass(sch.status)}" style="font-size:11px;">${sch.status}</span>` : `<span class="badge cancelled" style="font-size:11px;">Not Scheduled</span>`}
          </div>
        </div>
        <div class="panel-body">
          ...
          <!-- Time block -->
          ${sch ? `
            <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:rgba(196,154,60,0.07);border:1px solid var(--border);border-radius:10px;margin-bottom:16px;">
              <span style="font-size:20px;">&#x23F0;</span>
              <div>
                <div style="font-size:12px;color:var(--text-dim);text-transform:uppercase;letter-spacing:1px;">Deployment Period</div>
                <div style="font-size:15px;font-weight:700;color:var(--gold);">${sch.timePeriod?.label || '&#x2014;'}</div>
              </div>
            </div>` : `
            <div style="padding:12px 14px;background:rgba(220,38,38,0.06);border:1px solid var(--red);border-radius:10px;margin-bottom:16px;font-size:13px;color:var(--red);">
              &#x26A0; No equipment schedule assigned yet by admin. Contact your supervisor.
            </div>`}

          <!-- Staff group grid -->
          <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--gold);margin-bottom:10px;">Your Groups</div>
          <div class="sch-staff-groups" style="margin-bottom:${sch ? '20px' : '0'};">
            ${groups.map(g => `
              <div class="sch-staff-group-card">
                <div class="sch-staff-group-icon">${g.icon}</div>
                <div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:4px;">${g.label}</div>
                <div style="font-size:24px;font-family:'Playfair Display',serif;font-weight:700;color:${g.color};">${g.n}</div>
                <div style="font-size:10px;color:var(--text-dim);">staff members</div>
              </div>`).join('')}
          </div>

          <!-- Equipment list (if scheduled) -->
          ${sch && sch.items && sch.items.length ? `
            <div style="font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--gold);margin-bottom:10px;">Equipment to Deploy</div>
            <div style="display:flex;flex-wrap:wrap;gap:8px;">
              ${sch.items.map(it => {
                ...
                return `<div class="sch-item-pill" style="border-color:${clr};">
                  <span style="font-size:11px;font-weight:700;color:${clr};">${it.assignedQty}</span>
                  <span style="font-size:11px;color:var(--text-mid);">${it.name}</span>
                  ${!ok ? `<span style="font-size:10px;color:var(--red);">(need ${it.requiredQty})</span>` : ''}
                </div>`;
              }).join('')}
            </div>` : ''}
...
window.renderStaffSchedulingView = renderStaffSchedulingView;
