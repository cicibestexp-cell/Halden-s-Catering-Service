...
async function loadData() {
  ...
  try {
    ...
    debugDiv.style.cssText = 'position:fixed;bottom:10px;left:10px;background:rgba(0,0,0,0.8);color:white;padding:10px;font-family:monospace;z-index:9999;max-width:500px;font-size:10px;';
    
    let debugText = '--- DEBUG INFO ---\n';
    if (window.supabaseClient) {
      const { data, error } = await window.supabaseClient.auth.getSession();
      debugText += `Session user id: ${data?.session?.user?.id || 'null'}\n`;
      debugText += `Session error: ${error?.message || 'none'}\n`;
      
      const { data: resData, error: resErr } = await window.supabaseClient.from('reservations').select('id').limit(1);
      debugText += `Res query error: ${resErr?.message || 'none'} | rows: ${resData?.length}\n`;

      if (data?.session?.user?.id) {
         const { data: rsaData, error: rsaErr } = await window.supabaseClient.from('reservation_staff_assignments').select('*').eq('staff_id', data.session.user.id);
         debugText += `RSA query error: ${rsaErr?.message || 'none'} | rows: ${rsaData?.length}\n`;
      }
    } else {
      debugText += 'supabaseClient not found\n';
    }
...
function renderStaffDashboard() {
  ...
  if (assigned.length === 0) {
    ...
    // Helper: convert 24h "HH:MM" to 12-hr "H:MM AM/PM"
    const to12hr = (t) => {
      if (!t) return '';
    ...
    // Helper: parse timeframe string into two 24h tokens
    ...
    list.innerHTML = assigned.map(res => {
      ...
      const tfRaw = res.time || res.timeframe || '';
      const tf = parseTimeRange(tfRaw);
      const timeDisplay = tf ? `${to12hr(tf.s)} - ${to12hr(tf.e)}` : (tfRaw || '—');
      const address = res.venue || res.address || res.venueAddress || res.venueName || '—';
      return `<div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:20px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center;">
        <div>
          <div style="font-size:15px;font-weight:700;color:var(--gold);margin-bottom:4px;">${res.client || res.clientName || '—'}</div>
          <div style="font-size:12px;color:var(--cream);margin-bottom:2px;">${res.date || '—'} &middot; ${timeDisplay}</div>
          <div style="font-size:12px;color:var(--text-dim);">${address}</div>
        </div>
...
async function loadStaffProfile() {
  ...
  try {
    const { data, error } = await window.supabaseClient.from('users').select('*').eq('id', staff.id).single();
    if (error) {
    ...
    // Auto-switch availability if it's a new month
    if (data.last_availability_month !== currentMonth) {
      const { error: updateErr } = await window.supabaseClient.from('users').update({
        availability: 'Available',
        last_availability_month: currentMonth
      }).eq('id', staff.id);
    ...
    document.getElementById('prof-fname').textContent = data.fname || '—';
    document.getElementById('prof-mname').textContent = data.mname || '—';
    document.getElementById('prof-lname').textContent = data.lname || '—';
...
async function saveProfile() {
  ...
  const payload = {
    ...
    mname: document.getElementById('edit-mname').value.trim(),
    lname: document.getElementById('edit-lname').value.trim(),
    phone: document.getElementById('edit-phone').value.trim()
  };

  const { error } = await window.supabaseClient.from('users').update(payload).eq('id', staff.id);
  
  if (!error) {
    // Update local cache
    staff.name = `${payload.fname} ${payload.lname}`.trim();
...
async function initStaffCalendar() {
  ...
  try {
    ...
    finalRes.forEach(r => {
      ...
      if (!startStr) return; // Skip if date is invalid
      ...
      events.push({
        id: r.id,
        title: `${r.client || r.client_name || r.name || 'Client'} - ${r.type || r.packageName || r.occasion || 'Event'}`,
        start: startStr,
        allDay: !tStr,
