...
| **Customer Portal** | Embedded inside `index.html` | Logged-In Customers | Track reservations, attend meetings, view execution updates |
...
SMARTSERVE/
│
├── index.html                  # Public-facing landing page + customer portal (2001 lines, 113KB)
├── app.js                      # All frontend logic for index.html (5550 lines, 285KB)
├── style.css                   # Complete stylesheet for index.html (144KB)
...
├── customer.html               # Standalone customer-specific page (utility)
...
| Customer Dashboard | `#dash-overlay` | Full-screen overlay dashboard for logged-in customers |
...
- **Data Source:** `PKGS` array in `app.js` (also persisted to `localStorage` under key `halden_packages` for admin edits to reflect)
- **Packages (6 total):**

| Package ID | Name | Price/Head | Occasion | Pax Range |
|-----------|------|-----------|----------|-----------|
| `pkg-wedding` | Royal Wedding Package | ₱1,200 | Wedding | 100-200 |
| `pkg-debut` | Debut Grand Celebration | ₱950 | 18th Birthday | 80-150 |
| `pkg-kiddie` | Kiddie Wonderland | ₱650 | Birthday Party | 30-80 |
| `pkg-corp` | Corporate Elite | ₱850 | Corporate Event | 50-150 |
| `pkg-bday` | Grand Birthday Bash | ₱750 | Birthday | 50-120 |
| `pkg-dinner` | Intimate Soiree | ₱1,100 | Private Dinner | 20-50 |

- **Filtering:** By occasion category (Wedding, Birthday, Corporate, Private) + text search
- **Package Modal:** Clicking "View Details" opens a modal with package image, badge, theme, occasion, included items as pills, and an "Apply This Package" button
- **Applying a Package:** Loads all item IDs into the `customPkgItems` array, sets certain items (rice, low-cost desserts) as `isFree`, and redirects user to the Custom Package Builder section
...
Full-screen overlay dashboard (`#dash-overlay`) with sidebar and tabbed content:
