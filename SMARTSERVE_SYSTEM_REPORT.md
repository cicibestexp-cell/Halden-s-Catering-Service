...
## ⚠️ CRITICAL RULES FOR ALL AI ASSISTANTS — READ FIRST

> [!CAUTION]
> **RULE 1 — SCOPE: Do EXACTLY what the user asks, and NOTHING MORE.**
> Do NOT overcomplicate solutions. Do NOT make assumptions. Do NOT modify parts of the code you were not explicitly instructed to change. Keep fixes as simple and precise as possible. Failure to follow this rule will result in massive cascading errors and system breaks.
>
> *Exception:* You can add "smart functions" (e.g., preventing a user from inputting dates that have already passed) to improve UX and reliability, but you MUST always ask for permission first before implementing these instances.

> [!CAUTION]
> **RULE 2 — DATABASE: ALL database operations MUST use Supabase exclusively. Firebase is NOT to be used under any circumstances for any database, storage, or backend function whatsoever.**
> The codebase uses Firebase-style syntax (`window.firebaseDB`, `window.firebaseFns`, `addDoc`, `updateDoc`, `getDocs`, `onSnapshot`, etc.) purely as a compatibility alias layer defined in `supabase_adapter.js` — these calls are all routed to Supabase under the hood. Do NOT introduce any real Firebase SDK imports or direct Firebase API calls. If you need to add a new database table, collection, or query, it goes in Supabase only.

> [!CAUTION]
> **RULE 3 — UI STYLE: NEVER use emoticons/emojis anywhere in the UI. NEVER use colored oblong badge/pill designs for status or label indicators.**
> This means: no emoji characters (🍖, 📍, ⭐, 📅, ✨, ⏳, etc.) in any HTML, JS-generated HTML, or CSS content — in any portal, modal, card, or component. For status labels (e.g. "Confirmed", "Pending", "Rejected") and classification labels (e.g. "Majority Set", "Custom Build"), display them as plain uppercase gold text (`color: var(--cp-gold)` or equivalent) with letter-spacing — NO background fills, NO borders, NO border-radius pill/oblong shapes. Category section headers (Food, Drinks, Equipment, etc.) should also be plain uppercase gold text labels only.
...
### Live Package Modification (Real-Time Sync)
To support real-time package modification during a meeting without requiring the customer to refresh:
- The `is_modifying`, `live_draft`, `active_tab`, `meeting_notes`, and `is_live` columns were added to the Supabase `meetings` table.
- **Admin Side (`admin.js`)**: Modifying the package bypasses the Firebase adapter and directly updates Supabase (`is_modifying = true` and pushes the `live_draft` JSON object) because the Firebase adapter doesn't natively map these dynamically nested states for meetings. It strips currency formatting (₱ and commas) from the `activeMeetingResCache.amount` before mathematical operations via a custom `parseCurrency` utility to prevent `NaN` errors when syncing the total price.
- **Customer Side (`app.js`)**: Replaced the emulated `onSnapshot` adapter for the meeting room with a native **Supabase Realtime Channel** (`sb.channel(...).on('postgres_changes')`). This directly listens to the `meetings` table row and triggers a re-render (`updateCustomerMeetingUI`), instantly reflecting the admin's changes in a read-only catalog on the customer's screen.
...
## 18. Recent UI & Technical Updates Log

### Frontend & UI/UX Refinements (Mobile-First & Modernization)
- **Checkout Panel (`#chk-modal-box`)**: 
  - Converted from a centered pop-up modal to a sleek, right-side sliding drawer (matching the cart behavior). 
  - Replaced native emojis with sophisticated SVG icons. 
  - Added dedicated dark mode theming for a premium feel.
  - Fixed a bug where the `sourcePkg.time` object property was being incorrectly referenced as `timeframe`, resolving the Event Time Frame display issue during checkout.
- **Cart Drawer (`#cart-drawer`)**: 
  - Overhauled with the new elegant UI style matching the checkout drawer.
  - Fixed a layout bug where items were incorrectly restricted by a `max-width/clamp` CSS rule; items now properly stretch to 100% width and utilize `box-sizing: border-box`.
- **Premade Package Modal (`.pm-new-modal`)**: 
  - Upgraded the package overview pop-up to a structured, square-format modal utilizing `aspect-ratio` where appropriate but flexibly degrading to max-height scrolling on mobile.
  - Removed emojis in favor of elegant typography and SVG graphics.
- **Mobile Responsiveness (`style.css` max-width media queries)**:
  - Added full-width (`100vw`) support for the cart and checkout drawers on mobile devices (max-width: 768px).
  - Implemented specific edge-case fixes for very small devices (`max-width: 480px`) including vertical stacking of the package title/price elements, scaling down drawer paddings, reducing close button sizes, and stacking the theme/occasion indicators.
  - Applied robust Z-index management (`z-index: 4050` and `4100`) to guarantee correct overlay behavior of sliding drawers on mobile devices.

### Meeting Propose & Schedule Modal Fixes
- **Syntax Error Prevention**: Implemented safer `admin.js` structure after a truncated function (`openMtModal`) caused an `Unexpected end of input` crash.
- **Scrollbar Implementations**:
  - `Schedule Meeting` Panel: Applied hard constraints (`height: 650px`, `max-height: 650px`) to `.mt-modal-content` allowing `overflow-y: auto` to trigger successfully.
  - `Meeting Details` Panel: Added `max-height: 450px` to `.modal-body` to prevent infinite expansion.
- **Slot Limits**:
  - **Admin**: Hardcoded a limit of 3 proposed slots in `admin.js` (`addAdminProposeSlot`).
  - **Customer**: Hardcoded a limit of 3 preferred meeting slots in `app.js` (`addMeetingTimeSlot`).
- **Radio Selection Toggle logic**: Added logic to `toggleAdminPropose` that unchecks customer time radios when the admin proposes times, and re-selects the first customer time radio when the propose window is closed.

### Customer Portal Overhaul & Bug Fixes
- **Customer Meeting Hub Overlay (`customer.html`)**:
  - Injected the completely missing HTML structure for the `#c-mt-mode-overlay` (and child elements) which was causing the `Cannot set properties of null` crash when clicking "Join".
  - Structured the overlay as a strict "View-Only" UI. All interactive navigation tabs are disabled to ensure the customer only watches what the Admin controls.
- **FullCalendar Date Normalization (`app.js`)**:
  - Added a `normalizeDateKey()` function to properly convert text-based dates (e.g. `"Jul 31, 2026"`) from Supabase into ISO `YYYY-MM-DD` strings.
  - Fixed a silent failure where confirmed reservations were dropping off the customer calendar because FullCalendar rejected the string dates.
- **Emoji Removal & Aesthetic Cleanup (`customer.html` & `app.js`)**:
  - Executed a deep regex strip of all non-ASCII emoji characters across the customer portal to maintain a highly professional, text-based UI.
  - Restored core buttons with clean HTML text equivalents and arrows (`&rarr;`, `&larr;`, `&#10003;`).
  - Added `display: none;` to CSS classes (`.cp-stat-ico`, `.cp-mtg-ico`, `.cp-upcoming-ico`, `.cp-ni-ico`) to hide empty, blocky colored squares left behind by emoji removal.
  - Removed emojis injected directly via JavaScript into the customer calendar event titles.
- **Logout Sequence**: Fixed the customer sign-out button to properly call authentication wipe before redirecting to `index.html`.

### Meeting Cancellation Synchronization
- **Auto-Cancellation (`admin.js`)**:
  - Updated `updateReservationStatus()` so that when a reservation is cancelled/rejected, a `Promise.all` executes to instantly update all associated meetings in Supabase to `status: 'cancelled'`.
  - Enforced synchronous `await` logic to prevent race conditions where the UI would re-render before Supabase updated.
- **Orphaned Meeting Defense (`admin.js` & `app.js`)**:
  - Applied aggressive filtering to all meeting list renders (`renderWeeklyMeetings`, `initMtCalendar`, `renderCustomerMeetings`, `renderCustomerCalendar`).
  - Added a safety net that cross-references a meeting's `reservationId` against the reservations list. If the parent reservation is `cancelled`, the meeting is immediately hidden from the UI, retroactively fixing any old, orphaned meeting data.

### Supabase Adapter Fix — packageItems & city
- **Root Cause Identified**: `packageItems`, `city`, `rejectionReason`, and 20+ other reservation fields were not in the `allowedKeys` list in `supabase_adapter.js`, causing them to be silently buried inside `execution_plan._firebase_extras` JSONB and never properly retrieved.
- **Fix Applied**: Expanded `supabase_adapter.js` mapping layer to properly map all fields two-way (camelCase JS ↔ snake_case Supabase columns) and added corresponding `ALTER TABLE` migrations to the `reservations` table (`package_items`, `city`, `rejection_reason`, `active_pkg_id`, `contract_url`, `activity_logs`, `hired_personnel`, etc.).

### Initial Reservation Fee & PayMongo Integration
- **Admin UI Updates ( dmin.html &  dmin.js)**: 
  - Overhauled the Billing section in the Meeting Room to reflect a fixed ?5,000 initial reservation fee.
  - Implemented real-time trigger functionality (equestPaymongoPayment()) via Supabase to initiate the payment process on the customer's end.
  - Added robust manual confirmation handles: "Customer Paid Cash", "Did Not Pay" (24-hour deadline trigger), and a permanently visible "Cancel Request" button to fix stuck UI states without page refreshes.
- **Customer UI Updates ( pp.js)**:
  - Implemented `renderCustomerInitialFeeModal()` to dynamically spawn a full-screen, unskippable payment overlay the moment the admin initiates the fee.
  - Re-mapped the `startMeetingRoomListener` in `app.js` to correctly ingest `initial_fee_status`, `initial_fee_method`, `initial_fee_paid_at`, and `initial_fee_deadline` from the Supabase stream.
  - Integrated PayMongo checkout session creation directly inside the online payment selection.
  - Updated handlePaymentReturn to process the 	ype=initial_fee redirect callback, mark the reservation as paid, and sync the exact payment method and timestamp back into the meetings table in Supabase.
- **Backend Setup ( pi/paymongo.js)**:
  - Added payload handling for paymentType and dynamically routed PayMongo's success_url callback back to /?payment=success&type=initial_fee&meetingId=xxx to prevent exposing secret keys on the frontend.

### Downpayment UI & Meeting Initialization Crash Fix
- **Downpayment Calculation Overhaul (`admin.js`)**:
  - Replaced the brittle `calculateMtPrice` internal state calculation with `getReliableMtTotal`.
  - The new function scrapes the exact price being actively displayed in the DOM (`mt-mode-total-price`) as the absolute highest priority source of truth. This prevents stale caches from breaking the 50% calculation when dynamically-set packages are saved.
  - Added a safety bypass to skip DOM reading when `mtModifyMode` is active to prevent circular dependencies.
- **Customer UI Downpayment Display (`app.js`)**:
  - Fixed a bug where a previously broken 0-amount downpayment (saved to Supabase) would render as `₱0.00` on the customer side. It now actively ignores `0` and falls back to calculating 50% of the total on-the-fly.
- **Meeting Initialization Crash Fix (`admin.js`)**:
  - Fixed a silent failure in `initiateMeetingMode` where it was crashing due to a missing DOM element (`mt-mode-contract-btn-container`), which had been removed earlier to consolidate UI buttons into "Mark as Completed & Sign".
  - Added optional chaining and if checks to safely ignore missing redundant buttons, allowing the downstream downpayment initialization code to successfully run.
- **Function Scope Fix (`admin.js`)**:
  - Fixed an architectural failure where `window.updateBillingUI = function updateBillingUI` caused a ReferenceError for local internal calls within `initiateMeetingMode`. Converted back to a standard hoisted function declaration.


---

## Session Log -- July 20, 2026 (Evening)

The following bugs were identified and resolved during this session. All fixes were applied to production files.

---

### 1. Syntax Error in admin.js -- scheduleNextMeetingInMeetingMode

**File:** admin.js (line ~17228)
**Error:** Uncaught SyntaxError: Unexpected token '{'
**Root Cause:** A template literal in scheduleNextMeetingInMeetingMode was malformed. The timeEnd variable was missing and backtick delimiters were absent, leaving a bare dollar sign expression as raw code.

**Fix Applied:**
- Before: const time = (timeStart && timeEnd) ? dollar{timeStart} -  : '';
- After:  const time = (timeStart && timeEnd) ? backtick dollar{timeStart} - dollar{timeEnd}backtick : '';

---

### 2. Mojibake (Garbled Characters) Across UI -- admin.html, admin.js, app.js

**Error Observed:** Strings like garbled â€", âœ…, â‰¥, â–¾ appearing in log messages and throughout the UI in place of proper special characters (em-dashes, checkmarks, arrows, peso signs, etc.).
**Root Cause:** UTF-8 multi-byte character sequences had been double-encoded as Windows-1252 (CP1252). This happened when files were saved with the wrong encoding at an earlier point. The browser read valid UTF-8 but the underlying bytes in the source file were corrupted.
**Fix Applied:**
- Wrote and executed a Node.js script (fix_mojibake2.js) that used a regex to match 2-3 byte UTF-8 sequences mangled by CP1252 encoding (patterns starting with â, Â, Ã).
- The script re-decoded each mangled character by treating its char codes as raw CP1252 bytes then re-parsing them as proper UTF-8.
- All three files (admin.html, admin.js, app.js) were corrected and re-saved as clean UTF-8.
- Cleanup scripts (fix_mojibake.js, fix_mojibake2.js, fix.ps1) were deleted after use.

---

### 3. Customer Meeting Room White Screen -- app.js

**Error:** Uncaught (in promise) ReferenceError: draft is not defined at app.js:4520 and app.js:4568
**Root Cause:** Inside window.toggleCustomerMtPanel, two IIFE blocks referenced a "draft" variable that only exists inside renderCustomerLiveDraft. These code blocks had been incorrectly copied from the live-draft rendering context into the standard reservation panel renderer, where no "draft" object is in scope.

Affected lines:
- Line 4520: const origin = (draft.packageOrigin || res.packageOrigin || 'custom').toLowerCase();
- Line 4568: const origin = draft.packageOrigin || res.packageOrigin || 'custom';

**Fix Applied:** Removed all undefined draft.* references. Both lines now read only from res.packageOrigin with pricingMode as fallback:
- Line 4520: const origin = (res.packageOrigin || res.pricingMode || 'custom').toLowerCase();
- Line 4568: const origin = res.packageOrigin || res.pricingMode || 'custom';

Cache buster bumped: app.js v=13 -> v=14 in customer.html and index.html.

---

### 4. "Mark as Completed & Sign" Crash -- admin.js

**Error:** Error completing meeting: Could not find the 'contractUrl' column of 'meetings' in the schema cache
**Root Cause:** The completeMeeting function attempted to write contractUrl and uploadedFiles into the meetings table. The meetings table in Supabase does not have those columns -- contractUrl belongs only to the reservations table. Supabase rejected the entire update.
**Fix Applied:**
- Removed contractUrl and uploadedFiles from the meetings table update payload.
- contractUrl continues to be correctly saved to reservations via reservationPatch.contractUrl.
- The "View Snapshot Document" link in the meeting history panel was updated to fall back to the reservation's contractUrl when the meeting row itself does not have one.

---

### 5. "Payment Successful" Modal Reappearing Repeatedly -- app.js

**Error Observed:** The Payment Successful full-screen overlay appeared every time the customer meeting room UI re-rendered (triggered by every real-time Supabase snapshot update from the admin).
**Root Cause:** renderCustomerInitialFeeModal() is called on every onSnapshot event. Once initialFeeStatus becomes 'paid', the function rendered the success modal on every single call with no one-time suppression.
**Fix Applied:** Added a localStorage flag keyed by meeting ID:
- At the top of the paid branch: localStorage.setItem('fee_paid_' + meeting.id, 'true');
- Early return guard added before rendering: if (feeStatus === 'paid' && localStorage.getItem('fee_paid_' + meeting.id)) { if (modal) modal.remove(); return; }

The modal now shows exactly once (auto-dismisses after 6 seconds or on button click) and never reappears for the same meeting.

---

### 6. Package Origin Displaying as "CUSTOM" Instead of Correct Type

This bug had two independent root causes -- one on the customer side and one on the admin side.

#### 6a. Customer Meeting Room (app.js)

**Root Cause:** toggleCustomerMtPanel read res.packageOrigin to determine the package tier label. For reservations where packageOrigin was not explicitly set (only pricingMode existed in older records), the field was null, causing the fallback to 'custom'.
**Fix Applied:** Added res.pricingMode as an intermediate fallback at all three origin-check locations in the customer panel:
  const origin = (res.packageOrigin || res.pricingMode || 'custom').toLowerCase();

#### 6b. Admin Meeting Room -- Critical Missing Mapping in supabase_adapter.js

**Root Cause:** The mapFromDB() function in supabase_adapter.js -- which translates Postgres snake_case column names back into JS camelCase when reading from Supabase -- was entirely missing the reverse mapping for package_origin to packageOrigin.
- mapToDB() correctly saved packageOrigin as package_origin in the database.
- mapFromDB() never converted package_origin back to packageOrigin when reading.
- Therefore, even after correcting the database value to 'dynamically set', the admin meeting room's getMtOrigin() function received the field as package_origin (raw snake_case), found packageOrigin undefined, and always defaulted to 'custom'.

**Fix Applied:** Added the missing reverse mapping to mapFromDB() in supabase_adapter.js:
  if (fbData.package_origin !== undefined) { fbData.packageOrigin = fbData.package_origin; delete fbData.package_origin; }

Cache buster bumped: supabase_adapter.js v=3 -> v=4 in admin.html, index.html, and customer.html.

---

### 7. Schedule Next Meeting Saving Incorrect Customer Identity -- admin.js

**Error Observed:** When booking the next meeting from inside the admin meeting room, the created meeting document saved customerName as 'Client' and customerEmail as empty string, breaking the link to the actual customer account.
**Root Cause:** scheduleNextMeetingInMeetingMode attempted to read res.customerName and res.customerEmail. However:
- Reservations do not store a customerName field -- the client's name is under res.client.
- res.customerEmail is not always present; the correct field is res.email.
- Both lookups returned undefined, triggering the hardcoded fallback to the string 'Client'.

**Fix Applied:** Updated the function to read from activeMeeting (which has customerName and customerEmail synced when the meeting goes live) with proper cascading fallbacks. Also added clientId and clientName to match the standard meeting document schema used everywhere else in the system:
...
Cache buster bumped: admin.js v=7 -> v=8 in admin.html.

---

### Notes on Food Tasting / Next Meeting -- reservationId Linkage

Confirmed that all meetings created by scheduleNextMeetingInMeetingMode correctly include reservationId: activeMeeting.reservationId, which is the primary link used by:
- The customer portal (app.js) when querying all meetings for the customer's calendar and meeting hub.
- The admin reservation detail view when fetching meeting history.
- The real-time onSnapshot listener in customer.html that detects newly scheduled follow-up meetings.

Therefore, any Food Tasting, Design and Decorations, or Final Rundown meeting booked through the admin meeting room will automatically appear in the correct customer's reservation history and meeting hub view.

---

## 18. Recent Changelog

### Reservation Details Bug Fixes (Customer Portal)
- **Active Reservation Dropdown:** Updated the selector text in  pp.js to correctly display `r.packageName || r.type || 'Event'` instead of defaulting strictly to `r.type` ("Private").
- **Food Tasting Render Crash:** Fixed a scoping error in  pp.js (`renderFoodTastedTab`) where the `esc()` helper was undefined by introducing a localized `_escFt()` helper function.
- **Theme-Agnostic UI Variables:** Added mapped CSS variables (`--cream`, `--text-dim`, etc.) to the `:root` and `[data-theme="dark"]` blocks in customer.html. This ensures that shared rendering functions from  pp.js (which used admin-specific variables) fall back correctly to `var(--cp-text)` instead of rendering as invisible white text on the customer's light theme.
- **Initial Fee Default:** Modified  pp.js and  dmin.js to default the initial fee to ?5,000 if `paymentAmount` is falsy/0.
- **Operational Requirements Sync:** Injected the `window.ALLOC_RULES` dictionary directly into  pp.js before the `getReservationNeedsMarkup` function. This provides the customer portal with the necessary dependency to successfully compute and render operational requirements (tables, chairs, glassware) which was previously isolated to  dmin.js.

### Admin Dashboard Bug Fixes & UI Enhancements
- **Accurate Package Enforcement:** Updated the logic in `admin.js` to rigidly evaluate `package_origin` (from Supabase) instead of simply checking `isCustom`. This correctly grays out and marks restricted items as "LOCKED" (e.g. equipment and decorations) for dynamically and majorly set packages, matching the behavior in the Custom Package builder and Meeting Room.
- **Embedded Side-by-Side Catalog:** Relocated the catalog modification sidebar (`#resd-catalog-side`) natively into the `packageContent` variable generated by `renderReservationDetailContent()`. The catalog now sits cleanly next to the "Modifying Package Items" section inside a flex container rather than hovering awkwardly outside the main box.
- **Unfiltered Category Selection:** Removed strict array filtering on items arrays for dynamically set packages in `renderResdCatalog()`. All category buttons (Desserts, Equipment, Decorations, etc.) are now visible, allowing admins to properly view restricted items as "LOCKED" rather than hiding the categories altogether.
- **Sticky Layout Overflow Fix:** Changed `overflow-x: hidden` to `overflow-x: clip` on the `body` in `admin.css`. This prevents horizontal scrolling while natively supporting `position: sticky` on child elements inside the main content scrolling area.

### 19. UI Polish & Equipment Assets Enhancements (Current)
- **LiveKit Video Interface Fixes (`admin.js`, `admin.html`, `app.js`, `customer.html`):** Removed garbled text and broken emojis (e.g., `dYZT`, `dY"`) from admin controls and customer empty states. Adhered to strict branding by replacing customer video control emojis with plain text. Enforced `aspect-ratio: 16/9; min-width: 0;` on `.vcall-video-box` to fix disproportionate customer cameras. Refactored the identity generation to parse and display real user names from `window.currentUser` on the video tiles. Removed the cluttering `VC-XXXX` video room code badge from both portals.
- **Equipment Assets Fixes (`admin.js`, `admin.html`):** Resolved the `₱NaN` total asset value by injecting safe `parseFloat` and zero-fallbacks into `EIM_ASSETS.reduce`. Overhauled the Category filters, deleting hardcoded buttons from `admin.html` and introducing `updateEIMCategoryFilters()` to dynamically generate buttons based on unique category tags fetched live from the Supabase `equipment_inventory` table.
