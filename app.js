    ...
    return `
      <div class="cat-card pkg-catalog-card">
        ...
        <div class="cat-info">
          ...
          <ul class="pkg-inc-list">${p.items.map(it => `<li>${it}</li>`).join('')}</ul>
...
function renderCat() {
  ...
  if (!items.length) { grid.innerHTML = '<div class="cat-empty"><p>No items in this category.</p></div>'; return; }
...
function initSegmentedHero() {
  const track = document.getElementById('hero-segment-track');
  if (!track) return;
  track.innerHTML = HERO_IMAGES.map((img, i) =>
    `<div class="hero-segment ${i === 0 ? 'active' : ''}" style="background-image: url('${img.url}')"></div>`
  ).join('');
  let cur = 0;
  const items = track.querySelectorAll('.hero-segment');
  if (!items.length) return;
  function update() {
    ...
    const screenW = window.innerWidth;
    const itemW = items[cur].offsetWidth;
    const gap = 30; // matches CSS gap
    const step = itemW + gap;
    
    // Exact centering: (ScreenCenter) - (ItemsBefore * Step) - (ItemHalfWidth)
    const offset = (screenW / 2) - (cur * step) - (itemW / 2);
    track.style.transform = `translateX(\${offset}px)`;
  }
  // Initial positioning
  const initialOffset = (window.innerWidth / 2) - (items[0].offsetWidth / 2);
  track.style.transform = `translateX(\${initialOffset}px)`;
  
  // Slide every 10 seconds as requested
  setInterval(update, 10000);
}

window.addEventListener('load', () => {
  setTimeout(initSegmentedHero, 50);
