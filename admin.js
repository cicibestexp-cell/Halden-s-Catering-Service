    ...
    snap.forEach(doc => {
      ...
      if (!convoMap.has(msg.uid)) {
        convoMap.set(msg.uid, {
          ...
          lastMsg: msg.text,
          time: msg.timestamp,
          hasNew: msg.sender === 'customer' // simple logic for notif dot
        });
      }
    });
...
async function openConversation(uid, name) {
  ...
  activeChatUnsub = onSnapshot(q, (snap) => {
    ...
    snap.forEach(doc => {
      ...
      bubble.className = `chat-bubble ${msg.sender}`; // customer or staff
      bubble.innerHTML = `
...
async function sendAdminMsg() {
  ...
  try {
    const { collection, addDoc } = window.firebaseFns;
    // We get the customer info from the current active conversation if needed, 
    // but for reply we mainly need the uid.
    await addDoc(collection(window.firebaseDB, 'messages'), {
      uid: activeCustomerId,
      userName: document.querySelector('.ach-name').textContent, // keep name for listing
      text: text,
      sender: 'staff',
      timestamp: Date.now()
    });
...
function waitForFirebase(attempts = 0) {
  if (window.firebaseFns && window.firebaseDB) {
    loadData();
    initAdminChat(); // initialize inbox listening
  } else if (attempts < 20) {
...
Your goal is to analyze the provided business data (reservations, revenue, inventory) and provide 4 high-value strategic insights.
Format: Use HTML tags for structure. Each insight should start with a gold bullet point <span style='color:var(--gold);'>●</span> and a strong title.
...
async function generateAIReport() {
  ...
  // 1. Harvest Live Data for Context
  const confirmed = RESERVATIONS.filter(r => r.status === 'confirmed');
  const pending = RESERVATIONS.filter(r => r.status === 'pending');
  const criticalInv = INVENTORY.filter(i => i.status === 'critical' || i.status === 'low');
  
  const revTotal = confirmed.reduce((sum, r) => {
  ...
  const businessContext = `
    ...
    - Critical Inventory Items: ${criticalInv.map(i => `${i.name} (${i.stock} ${i.unit})`).join(', ') || 'None'}
    - Top Event Types: ${[...new Set(confirmed.map(r => r.type))].join(', ')}
  `;

  try {
    // 2. Call the real Arcee AI Model via Vercel Proxy
    const response = await fetch(AI_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Please generate a business intelligence report based on this data: ${businessContext}` }
        ]
      })
    });

    const data = await response.json();
    const reply = data.choices?.[0]?.message?.content || "I was unable to synthesize the data at this moment. Please try again.";
    ...
    body.innerHTML = `<p style="color:var(--red);">🚨 Error connecting to Intelligence Engine. Please check your network or Vercel API status.</p>`;
    container.style.display = 'block';
  } finally {
    btn.disabled = false;
